...
// test4: same scene as test3, but the TREE layer comes from aerial detection (detect-trees.mjs)
// instead of hand-authored per-building tree props — the "aerial = WHERE" proof.
const OUT = arg('out', path.join(path.dirname(W.paths.assetGlb), 'world_test4.glb'));
...
const IDS = (arg('ids', '') || SCENE.ids.join(',')).split(',').filter(Boolean);
const AERIAL_TAG = arg('trees', 'tjunction');
...
for (const ID of IDS) {
  ...
  if (!b || !fs.existsSync(progPath)) { console.error('skip ' + ID + ' (no building/program)'); continue; }
  ...
  const scRoad = SCENE.roads.find(x => x.id === r.id);
  ...
  frames.push({ id: ID, cx: +o.cx.toFixed(1), cz: +o.cz.toFixed(1),
    roadId: r.id, roadType: r.type, roadDist: +r.d.toFixed(1),
    halfW: scRoad ? scRoad.halfW : 3.4, surface: scRoad ? scRoad.surface : 'asphalt',
    off: +off.toFixed(2), L: +Math.hypot(frontage.b[0] - frontage.a[0], frontage.b[1] - frontage.a[1]).toFixed(1),
    frontA: [+frontage.a[0].toFixed(1), +frontage.a[1].toFixed(1)],
    frontB: [+frontage.b[0].toFixed(1), +frontage.b[1].toFixed(1)],
    inward: [+(-frontage.nx).toFixed(3), +(-frontage.nz).toFixed(3)],
    hasEnv: !!program.environment });
  console.log(`  bld ${ID} at (${o.cx.toFixed(1)}, ${o.cz.toFixed(1)}) road ${r.id}/${r.type} d=${r.d.toFixed(1)} off=${off.toFixed(1)} env=${!!program.environment}`);
}
fs.writeFileSync(path.join(W.paths.specWork, 'tjunction-frames.json'), JSON.stringify(frames, null, 1));
console.log(`[${W.name}] test3: ${jobs.length} buildings; spawn suggestion: ?test3&spawnat=115,-290`);
...
const sceneRoads = SCENE.roads.map(sc => {
  const rd = ROADS.find(x => String(x.id) === String(sc.id));
  return { ...sc, pts: clipPts(rd.pts, sc.clipLen) };
});

// Cull aerial trees that fall in a roadway/kerb (island grass + parked-car strips read as
// canopy from above). Done in node where nearestRoad + scene half-widths are available.
const AERIAL_TREES = TREES.filter(t => {
  const r = nearestRoad(t.x, t.z);
  const scR = SCENE.roads.find(x => x.id === r.id);
  return r.d >= ((scR ? scR.halfW : 3.4) + 0.6);
});
console.log(`[netanya] test4: aerial trees ${AERIAL_TREES.length}/${TREES.length} kept after roadway cull`);
